...
export default function Property() {
  ...
  const sendEmail = (e) => {
    e.preventDefault();
    const finalMessage = message.trim() || `I am interested in your property '${property?.title}'.`;
    ...
    return (
      <Layout>
        <div className='h-3/4 w-screen text-center'>
          <BeatLoader color="#064862" size={30} aria-label="Loading..." data-testid="loader" />
        </div>
      </Layout>
    );
  }
          ...
          {property ? (
            <div className="lg:w-8/12 mx-auto">
              <div className="flex justify-between items-center w-full relative mb-4">
                ...
                {userInfo?.data?._id === property?.ownerId?.id ? (
                  <div className="absolute top-0 right-0 flex gap-3">
                    ...
                    <Link className="text-white bg-green-600 py-2 px-5 hover:bg-gray-500 rounded text-2xl" to={`/edit-property/${property.id}`}><SlNote /></Link>
                  </div>
                ) : (
                  <div className="absolute top-0 right-0 flex gap-3">
              ...
              <div className="h-96 my-2">
                <img src={property.imageUrl} alt="Property" className="object-cover object-center h-full w-full rounded-md shadow-md" />
              </div>
              <div className="flex flex-col sm:flex-row">
                <div className="sm:w-2/3 sm:pr-4 sm:py-8 mt-4 sm:mt-0 border-b border-gray-200">
                  ...
                  <div className='p-4 bg-slate-50 rounded-md shadow-md mb-7'>
                    <h2 className="title-font font-medium text-lg text-gray-900 mb-5">Description</h2>
                    <p>{property.description}</p>
                    <Link className="text-secondary inline-flex items-center">Learn More</Link>
                  </div>

                  {/* Property Details */}
                  <div className="p-4 bg-slate-50 rounded-md shadow-md mb-7">
                    <h2 className="title-font font-medium text-lg text-gray-900 mb-5">Property Details</h2>
